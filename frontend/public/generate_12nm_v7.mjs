  ...
  try {
    ...
    const subtractWarRisk = (poly) => {
        ...
        for (const wrz of warRiskData.features) {
            if (!currentPoly) break;
            try {
                // If the bounding boxes intersect, calculate the difference
                const polyBox = turf.bboxPolygon(turf.bbox(currentPoly));
                const wrzBox = turf.bboxPolygon(turf.bbox(wrz));
                if (turf.booleanOverlap(polyBox, wrzBox) || turf.booleanWithin(polyBox, wrzBox) || turf.booleanWithin(wrzBox, polyBox)) {
