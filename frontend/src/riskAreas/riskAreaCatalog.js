...
const BASELINE_AMENDMENT_MIGRATION_KEY = "__jwlaBaselineAmendmentVersion";
const BASELINE_AMENDMENT_MIGRATION_VERSION = 1;

export function migrateRiskAreaSettings(settings = {}) {
  if (settings?.[BASELINE_AMENDMENT_MIGRATION_KEY] === BASELINE_AMENDMENT_MIGRATION_VERSION) {
    return settings;
  }

  const next = { ...settings };
  for (const sectionId of ["contract-alerts", "jwc-034-amendment"]) {
    const section = RISK_AREA_SECTIONS.find((candidate) => candidate.id === sectionId);
    for (const areaItem of section?.items || []) {
      for (const layerKey of areaItem.layerKeys || []) {
        next[layerKey] = getLayerDefaults(layerKey);
      }
    }
  }
  next[BASELINE_AMENDMENT_MIGRATION_KEY] = BASELINE_AMENDMENT_MIGRATION_VERSION;
  return next;
}
